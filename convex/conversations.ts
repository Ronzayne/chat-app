    ...
    const conversationsWithDetails = await Promise.all(
      myConversations.map(async (conversation) => {
        ...
        if (!conversation.isGroup) {
          // if it is not a group then we are fetching the other users profile
        ...
        const lastMessage = await ctx.db
          ...
          .order("desc") // in order to return the latest messages
          .take(1);

        // return should be in this order, otherwise _id field will be overwritten
        return {
          ...userDetails,
          ...conversation, //should be like this because the conversationID should overwrite the userId
