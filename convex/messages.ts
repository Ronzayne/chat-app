...
export const sendTextMessage = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const conversation = await ctx.db
      .query("conversations")
      .filter((q) => q.eq(q.field("_id"), args.conversation)) // finding the conversation that the id matches with the conversation id being taken as the arguement
      .first();
    ...
    await ctx.db.insert("messages", {
      // if a the if checks are passed then this should be inserted in the messages table
      sender: args.sender,
...
export const getMessages = query({
  args: {
    conversation: v.id("conversations"), // get the messages only for one particular conversation
  },
  handler: async (ctx, args) => {
    const identity = await ctx.auth.getUserIdentity();
    if (!identity) {
